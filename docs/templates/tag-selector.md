...
일반 Blade 폼에서는 기존 태그 id와 신규 태그 후보명이 hidden input으로 전송된다.
...
Livewire 부모 컴포넌트에서는 `valueMode`에 따라 부모 property로 바로 동기화된다.

| 모드 | 부모 값 | 예시 |
| --- | --- | --- |
| `ids` | 기존 태그 id 배열 | `[1, 2]` |
| `names` | 기존/신규 태그명 배열 | `['청소', '욕실정리']` |

저장 데이터로 정리할 때는 기존 태그 id를 `tagIds`, 신규/AI 태그명을 `tagNames`로 다룬다. `TipDraftData`는 입력 배열에서 `tag_ids`, `tagIds`, `tag_names`, `tagNames`, `tags` 키를 받아 내부의 `tagIds`와 `tagNames`로 정규화한다.
...
```mermaid
flowchart TD
    A["선택된 태그 값"] --> B["tagIds와 tagNames로 정규화"]
